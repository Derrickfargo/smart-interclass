...
public final class BufferUtils {
	private static Logger logger = Logger.getLogger(BufferUtils.class.getName());
	 ...
	 *            接收的byte数组
	 * @param val
	 *            需要转化的整数，若需要将int或者short型数据填入4字节和2字节的byte数组中，则需强转为long类型数据再调用此方法
	 * @param offset
	 *            默认设置为0
	 * @param size
	 *            结束位置，根据数据类型来定。long型则设置为8，int类型设置为4，short类型设置为2
	 */
	public static void encodeIntLittleEndian(byte[] dst, long val, int offset,
			int size) {
	...
	public static int byteToInt2(byte[] b, int offset) {
		...
		return (ch1 << TWENTY_FOUR) + (ch2 << SIXTEEN) + (ch3 << EIGHT)
				+ (ch4 << 0);
	}
	...
	public static byte[] writeUTFString(String str) {
		...
		try {
			...
			logger.info("不支持字符编码:" + e.getMessage());
		}
		return byts;
	}
