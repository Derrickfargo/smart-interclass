...
public class MessageParser {
	...
	public void parseMessage(SelectionKey key) {
		...
		try {
			...
			MyApplication.Logger.debug(AndroidUtil.getCurrentTime()
					+ "MessageParser.class" + ":获取MessageHandler出错:", e);
			return;
		}
		headerBuffer.flip();
		MyApplication.Logger.debug(AndroidUtil.getCurrentTime()
				+ "MessageParser.class" + "开始解析消息...");
		// 消息头fakeId是否正确
	...
	private boolean parseFakeId(ByteBuffer buffer) {
		...
		try {
			...
			// 获取fake id的值
			fakeId = Integer.parseInt(BufferUtils.decodeIntLittleEndian(
					fakeIdByte, 0, fakeIdByte.length) + "");
		} catch (NumberFormatException ex) {
			readRemain();
			ApiClient.uploadErrorLog(ex.getMessage());
			ex.printStackTrace();
			MyApplication.Logger.debug(AndroidUtil.getCurrentTime()
					+ "MessageParser.class:" + "ilegal Number parser");
			return false;
		} catch (Exception e) {
			readRemain();
			ApiClient.uploadErrorLog(e.getMessage());
			MyApplication.Logger.debug(AndroidUtil.getCurrentTime()
					+ "MessageParser.class:" + "unknow fake Id parser failed");
			return false;
		}
	...
	private boolean parseMsgBody() {
		...
		try {
			...
			readRemain();
			ApiClient.uploadErrorLog(e.getMessage());
			MyApplication.Logger.debug(
					AndroidUtil.getCurrentTime() + "MessageParser.class:"
							+ "failed to fetch message body :", e);
			return false;
		}
	}
	...
	public void readRemain() {
		MyApplication.Logger.debug("消息解析出现异常 读完所有内容");
		try {
			ByteBuffer mHeaderBuffer = BufferUtils.prepareToReadOrPut(1024);
			while (channel.read(mHeaderBuffer) != 0) {
				channel.close();
				mHeaderBuffer.clear();
				channel.read(mHeaderBuffer);
			}
		} catch (IOException e) {
			e.printStackTrace();
		};
