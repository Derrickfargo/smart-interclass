 ...
 */
package com.popoy.widget.table;

import java.util.List;

import com.popoy.widget.listener.AbOnItemClickListener;
 ...
 * AbCellType.STRING, AbCellType.STRING,AbCellType.STRING};
 * <p>
 * (4)列宽配置(%) 超过100% 可以横向滑动
 * <p>
 ...
 * <p>
 * tableResource = new int[]
 * {android.R.color.transparent,R.drawable.title_cell,android
 * .R.color.transparent,R.drawable.content_cell};
 * <p>
...
public class AbTable {
	...
	public static AbTable newAbTable(Context context, int columnSize) {
		...
		// 列类型配置
		mAbTable.titleCellTypes = new int[columnSize];
		for (int i = 0; i < columnSize; i++) {
			mAbTable.titleCellTypes[i] = AbCellType.STRING;
		}
		...
		// 行文字颜色（索引0标题，1内容列表）
		mAbTable.rowTextColor = new int[] { Color.rgb(113, 113, 113),
				Color.rgb(113, 113, 113) };
		// 背景资源（索引0标题行背景，1标题单元格背景，2内容列表行背景。3表格内容单元格背景）
		mAbTable.tableResource = new int[] { android.R.color.transparent,
	...
	/**
	 * gets the title cell types
	 * 
	 * @return
	 */
	public int[] getTitleCellTypes() {
		return titleCellTypes;
	}

	/**
	 * 描述：列类型配置
	 * 
	 * @param titleCellTypes
	 */
	public void setTitleCellTypes(int[] titleCellTypes) {
		this.titleCellTypes = titleCellTypes;
	}
	 ...
	 * 
	 * @param itemCellTouchListener
	 *            the new item cell touch listener
	 */
	public void setItemCellTouchListener(
	 ...
	 * 
	 * @param itemCellCheckListener
	 *            the new item cell check listener
	 */
	public void setItemCellCheckListener(
