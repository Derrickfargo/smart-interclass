 ...
 */
package com.popoy.tookit.cache;
...
import com.popoy.tookit.utils.AndroidVersionCheckUtils;
...
public class TAFileCache
{
				...
				if (mCacheParams.diskCacheEnabled && diskCacheDir != null)
				{
					...
					if (usableSpace > mCacheParams.diskCacheSize)
					{
						...
						} catch (final IOException e)
						{
							...
							Logger.e(TAFileCache.this, "initDiskCache - " + e);
						}
					}
				}
			}
	...
	public void addBufferToCache(String data, byte[] buffer)
	{
		...
		synchronized (mDiskCacheLock)
		{
			...
			if (mDiskLruCache != null)
			{
				...
				{
					Logger.e(TAFileCache.this, "addBufferToCache - " + e);
				} catch (Exception e)
				{
					Logger.e(TAFileCache.this, "addBufferToCache - " + e);
				} finally
	...
	public byte[] getBufferFromMemCache(String data)
	{
		...
		} catch (Exception e)
		{
			...
			e.printStackTrace();
			Logger.e(TAFileCache.this, "获取缓存数据失败！");
		}
		return memValue;
	}
		...
		synchronized (mDiskCacheLock)
		{
			...
			if (mDiskLruCache != null)
			{
				...
				} catch (final IOException e)
				{
					...
					Logger.e(TAFileCache.this, "getBufferFromDiskCache - "
							+ e);
				} catch (Exception e)
				{
	...
	public void clearCache()
	{
		...
		synchronized (mDiskCacheLock)
		{
			...
			if (mDiskLruCache != null && !mDiskLruCache.isClosed())
			{
				...
				{
					Logger.e(TAFileCache.this, "clearCache - " + e);
				}
				mDiskLruCache = null;
				initDiskCache();
			}
	...
	public void flush()
	{
		synchronized (mDiskCacheLock)
		{
			if (mDiskLruCache != null)
			{
				...
				{
					Logger.e(TAFileCache.this, "flush - " + e);
				}
			}
		}
	}
	...
	public void close()
	{
		synchronized (mDiskCacheLock)
		{
			if (mDiskLruCache != null)
			{
				...
				{
					Logger.e(TAFileCache.this, "close" + e);
				}
			}
		}
	}
