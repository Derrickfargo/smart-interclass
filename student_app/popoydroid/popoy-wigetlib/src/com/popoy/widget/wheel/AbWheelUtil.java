 ...
 */
package com.popoy.widget.wheel;
...
import com.popoy.tookit.utils.DateTimeUtil;
import com.popoy.tookit.utils.StringUtils;
...
public class AbWheelUtil {
	 ...
	 */
	public static void initWheelDatePicker(final TAActivity activity,final TextView mText,final AbWheelView mWheelViewY,final AbWheelView mWheelViewM,final AbWheelView mWheelViewD,
		...
		AbOnWheelChangedListener wheelListener_year = new AbOnWheelChangedListener() {
			...
			public void onChanged(AbWheelView wheel, int oldValue, int newValue) {
				...
				if (list_big.contains(String.valueOf(mWheelViewM.getCurrentItem() + 1))) {
					...
					if (DateTimeUtil.isLeapYear(year_num))
						mWheelViewD.setAdapter(new AbNumericWheelAdapter(1, 29));
					else
						mWheelViewD.setAdapter(new AbNumericWheelAdapter(1, 28));
				}
		...
		okBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				activity.removeDialog(DIALOGBOTTOM);
	 ...
	 * @version v1.0
	 */
	public static void initWheelTimePicker(final TAActivity activity,final TextView mText,final AbWheelView mWheelViewMD,final AbWheelView mWheelViewHH,final AbWheelView mWheelViewMM,
			 Button okBtn,Button cancelBtn,
		...
		for(int i=1;i<13;i++){
			if(list_big.contains(String.valueOf(i))){
				...
				if(i==2){
					if(DateTimeUtil.isLeapYear(defaultYear)){
						for(int j=1;j<28;j++){
							textDMList.add(i+"月"+" "+j+"日");
							textDMDateList.add(defaultYear+"-"+i+"-"+j);
						}
		...
		// 月日
		mWheelViewMD.setAdapter(new AbStringWheelAdapter(textDMList,StringUtils.strLength("12月"+" "+"12日")));
	 ...
	 * @param initStart the init start
	 */
	public static void initWheelTimePicker2(final TAActivity activity,final TextView mText,final AbWheelView mWheelViewHH,final AbWheelView mWheelViewMM,
			 Button okBtn,Button cancelBtn,
		...
		String val = StringUtils.dateTimeFormat(defaultHour+":"+defaultMinute) ;
		mText.setText(val);
